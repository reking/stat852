...
#  x1 and x2 are U(0,1); x3=10 * X1 + unif(0,1).  
...
# Plot the "coefficient path": Values of the parameter estimates as lambda changes.
#  Note that these are not in the original scale.
plot(ridge)
...
plot(ridgec)
select(ridgec)
coef(ridgec)[which.min(ridgec$GCV),]
...
pred.olsc <- predict(olsc,newdata=test)
