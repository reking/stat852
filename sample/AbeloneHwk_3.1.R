...
lasso.cv5 <- crossval(x=x.1, y=y.1, theta.fit=lasso.fit, theta.predict=lasso.pred, ngroup=5)
win.graph(h=12, w=12)
...
allsub.cols <- which(summary(fit)$which[order.bic[1],]==TRUE)
allsub.cols1 <- allsub.cols[-1]-1
allsub.x1 <- x.1[,allsub.cols1]
allsub.x23 <- x.23[,allsub.cols1]
beta <- coef(allsub,id=order.bic[1])
allsub.pred.x1 <- cbind(1,allsub.x1)%*%beta 
allsub.pred.x23 <- cbind(1,allsub.x23)%*%beta 
mspe.allsub.x23 <- mean((y.23 - allsub.pred.x23)^2)
