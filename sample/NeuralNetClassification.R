...
library(e1071)
#######################
# Sadly, I can't make the tune() function work for this multiclass nnet.
######################
tuned <- tune(method="nnet", train.y=y.1, train.x=x.1, validation.y=y.2, validation.x=x.2, 
          ranges=list(size=c(1,5,10), decay=10^c(-3,-2,-1)), tunecontrol=tune.control(nrepeat=5), maxit=600, softmax=TRUE)
tuned <- tune.nnet(y=y.1 , x=x.1, size=c(1,5,10), decay=c(.001,.01,.1), maxit=600, softmax=TRUE)
#tuned <- tune.nnet(data=set1, formula=class ~ Compactness + Circularity +	Distance.Circularity 
...
#                   size=c(1,5,10), decay=c(.001,.01,.1), maxit=600, softmax=TRUE)
...
nn.15.001 <- nnet(x=x.1, y=y.1, size=15, maxit=2000, decay=.001, softmax=TRUE)
