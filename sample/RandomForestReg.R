...
air.rf <- randomForest(data=AQ, Ozone~Wind+Temp, 
...
air.oob <- mean((predict(air.rf) - AQ$Ozone)^2)
...
par(mfrow=c(1,2))
partialPlot(air.rf, pred.data=AQ, x.var=Temp)
points(y=AQ$Ozone, x=AQ$Temp)
partialPlot(air.rf, pred.data=AQ, x.var=Wind)
points(y=AQ$Ozone, x=AQ$Wind)
...
win.graph(h=7,w=12,pointsize=12)
par(mfrow=c(1,2))
partialPlot(air.rf2, pred.data=AQ, x.var=Temp)
partialPlot(air.rf2, pred.data=AQ, x.var=Wind)
win.graph(h=7,w=6,pointsize=12)
plot(air.rf2)
win.graph(h=7,w=6,pointsize=12)
hist(treesize(air.rf2), breaks=c(15:21))
varUsed(air.rf2)
air.oob2 <- mean((predict(air.rf2) - AQ$Ozone)^2)
...
# Do smaller trees help here?  Nope!
air.rf3 <- randomForest(data=AQ, 
                        Ozone~Wind+Temp, importance=TRUE, ntree=1000, 
                        mtry=1, keep.forest=TRUE, maxnodes=10)
air.oob3 <- mean((predict(air.rf3) - AQ$Ozone)^2)
