...
for(i in 1:20)
{
  ...
  x.1.unscaled <- as.matrix(set1[which(set1$bina==1),1:6])
  y.1 <- class.ind(set1[which(set1$bina==1),7])
  y.r <- set1[which(set1$bina==1),7]
  x.2.unscaled <- as.matrix(set1[which(set1$bina==2),1:6])
  y.2 <- class.ind(set1[which(set1$bina==2),7])
